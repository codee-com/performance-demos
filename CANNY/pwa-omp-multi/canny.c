        ...
        short int **smoothedim)
{
   ...
   ****************************************************************************/
   ...
   #pragma omp parallel default(none) shared(center, cols, image, kernel, rows, tempim) private(c, cc, dot, r, sum)
   {
   #pragma omp for private(c, cc, dot, sum) schedule(auto)
   for(r=0;r<rows;r++){
      for(c=0;c<cols;c++){
         dot = 0.0;
         sum = 0.0;
   ...
   ****************************************************************************/
   ...
   #pragma omp parallel default(none) shared(center, cols, kernel, rows, smoothedim, tempim) private(c, dot, r, rr, sum)
   {
   #pragma omp for private(dot, r, rr, sum) schedule(auto)
   for(c=0;c<cols;c++){
      for(r=0;r<rows;r++){
         sum = 0.0;
         dot = 0.0;
